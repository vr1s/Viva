...
@implementation HPPageConfiguration
{

@private
    HPLayoutConfiguration _layoutConfiguration;
    HPLayoutOptions _layoutOptions;
    NSString *name;
}
...
- (HPPageConfiguration *)initWithCoder:(NSCoder *)coder
{
    self = [super init];
    if (self)
    {
        ...
        HPLayoutConfiguration config = {.pageInsets = pageInsets, .pageSpacing = pageSpacing, .iconGridSize = iconGridSize, .iconImageInfo = iconImageInfo};
        HPLayoutOptions opts = {.hideBadges=hideBadges, .hideLabels = hideLabels, .hidePageControl = hidePageControl};
