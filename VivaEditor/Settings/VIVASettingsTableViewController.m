//
// VIVASettingsTableViewController.m
// Viva
//
...
@implementation VIVASettingsTableViewController
...
- (void)viewDidLoad
{
    ...
    [self.tableView registerClass:[VIVATableCell class] forCellReuseIdentifier:@"Cell"];
...
-(void)doneButtonPressed:(UIButton*)butt 
{
    [self dismissViewControllerAnimated:YES completion:^{
            [[[VIVAUIManager sharedInstance] editorViewController] handleDoneSettingsButtonPress:nil];
        }];
}
...
- (BOOL)storedStateForSwitchAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return [[VIVAConfigurationManager.sharedInstance.currentConfiguration pageConfigurations][iconLocation] boolForOption:key];
}

- (VIVATableCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([_rtCells[(NSUInteger) indexPath.section][@"Items"][(NSUInteger) indexPath.row][@"Cell"] isEqualToString:@"Default"])
    {
        VIVATableCell *cell = [tableView dequeueReusableCellWithIdentifier:@"SettingsPaneCell"];

        if (!cell) 
        {
            cell = [[VIVATableCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"SettingsPaneCell"];
    ...
    VIVATableCell *cell = [tableView dequeueReusableCellWithIdentifier:@"SettingsPaneSwitchCell"];

    if( cell == nil )
    {
        cell = [[VIVATableCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"SettingsPaneSwitchCell"];
        cell.textLabel.text = _rtCells[(NSUInteger)indexPath.section][@"Items"][(NSUInteger) indexPath.row][@"Title"];
...
- (void)settingSwitchChanged:(UISwitch *)sender
{
    ...
    [[VIVAConfigurationManager.sharedInstance.currentConfiguration pageConfigurations][iconLocation] setBool:sender.on
                                                            forOption:key];

    [kIconModel layout];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"VIVAUpdateLayoutCache" object:nil];
                ...
                case 0: 
                {
                    UIAlertController * alert = [UIAlertController
                                    alertControllerWithTitle:@"This will reset your current loadout!"
                                                    message:[VIVAUtility localizedItem:@"ARE_YOU_SURE_RESET"]
                                            preferredStyle:UIAlertControllerStyleAlert];
                    UIAlertAction* yesButton = [UIAlertAction
                                        actionWithTitle:@"Yes"
                                                style:UIAlertActionStyleDefault
                                                handler:^(UIAlertAction * action) {
                                                    [VIVAConfigurationManager.sharedInstance deleteConfigurationWithName:VIVAConfigurationManager.sharedInstance.directory.selectedConfigurationName];
                                                    [VIVAConfigurationManager.sharedInstance setCurrentConfiguration:[VIVAConfigurationManager.sharedInstance configurationWithName:VIVAConfigurationManager.sharedInstance.directory.selectedConfigurationName createIfNecessary:YES]];
                ...
                case 1: 
                {
                    ...
                    UIAlertAction* yesButton = [UIAlertAction
                                        actionWithTitle:@"Yes"
                                                style:UIAlertActionStyleDefault
                                                handler:^(UIAlertAction * action) {
                                                    [VIVAConfigurationManager.sharedInstance reset];
                                                    [VIVALayoutManager.sharedInstance initializeCacheOverride];
                                                    [kIconModel layout];
                                                    [[[VIVAUIManager sharedInstance] editorViewController] reload];
                                                }];
...
NSArray *getCells()
{
    return @[
            @{
                    @"Cell":@"Section",
                    @"Title":[VIVAUtility localizedItem:@"LOADOUT"],
                    @"Items":@[
                    @{
                            @"Cell":@"Default",
                            @"Title":[VIVAUtility localizedItem:@"RESET_VALUES"],
                            @"Key":@"RESETVALUES"
                    },
                    @{
                            @"Cell":@"Default",
            ...
            @{
                    ...
                    @{
                            @"Cell":@"Switch",
                            @"Title":[VIVAUtility localizedItem:@"HIDE_ICON_LABELS"],
                            @"Key":@"HideLabels"
                    },
                    @{
                            @"Cell":@"Switch",
                            @"Title":[VIVAUtility localizedItem:@"HIDE_BADGES"],
                            @"Key":@"HideBadges"
                    },
                    @{
                            @"Cell":@"Switch",
            ...
            @{
                    ...
                    @{
                            @"Cell":@"Default",
                            @"Title":[VIVAUtility localizedItem:@"Respring"],
                            @"Key":@"RESPRING"
                    }]
            }];
}