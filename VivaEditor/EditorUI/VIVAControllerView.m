...
@implementation VIVAControllerView
{
@private
    VIVAControllerViewConfiguration _config;
}
...
- (void)layoutControllerView
{
    ...
    UIImage *rsImage = [VIVAResources resetImageToggled:NO];
    ...
    location.text = [[[VIVAUIManager sharedInstance] editingLocation] substringFromIndex:14];
    location.textAlignment = NSTextAlignmentRight;
    [location setFont:[UIFont systemFontOfSize:13]];
}

- (void)updateLayoutForConfiguration:(VIVAControllerViewConfiguration)config
{

    self.topLabel.text = [VIVAUtility localizedItem:config.topControl.itemInfo.label];
    self.bottomLabel.text = [VIVAUtility localizedItem:config.bottomControl.itemInfo.label];
    ...
    {
        case kVIVAControllerItemTypeNone:
            _topView.hidden = YES;
            break;
        case kVIVAControllerItemTypeCounter:
        {
            self.topLabel.frame = CGRectMake(kLeftScreenBuffer * [[UIScreen mainScreen] bounds].size.width, -10, (0.706) * [[UIScreen mainScreen] bounds].size.width, (0.0615) * [[UIScreen mainScreen] bounds].size.height);
    ...
    VIVAPageConfiguration *pageConfig = [VIVAConfigurationManager.sharedInstance.currentConfiguration pageConfigurations][[[VIVAUIManager sharedInstance] editingLocation]];
...
{
    [VIVALayoutManager updateConfigItem:_config.topControl.itemInfo.configKey
                          forLocation:[[VIVAUIManager sharedInstance] editingLocation]
...
{
    [VIVALayoutManager updateConfigItem:_config.bottomControl.itemInfo.configKey
                          forLocation:[[VIVAUIManager sharedInstance] editingLocation]
                            withValue:(NSInteger) slider.value];
